        ...
        <el-card :body-style="{ padding: '20px' }" class="product-item">
          ...
          <div class="product-details">
            ...
            <div class="actions">
              <!-- 下单按钮 -->
              <el-button
                @click="orderProduct(product.name)"
                size="large"
  ...
  methods: {
    ...
    // let url = "http://10.100.164.35:8080/products";
        ...
        axios
          ...
          .then((response) => {
            ...
            if (this.products.length > 0) {
              this.sendUserBehavior("search", this.products[0].name);
            }
          })
          .catch((error) => {
      ...
      //  url = `http://10.100.164.35:8080/products/category/${encodeURIComponent(
        url = `http://localhost:8080/products/category/${encodeURIComponent(
    ...
    addToCart(name) {
      this.sendUserBehavior("addToCart", name);
      this.$message.success("商品已加入购物车");
    },
    orderProduct(name) {
      this.sendUserBehavior("purchase", name);
    ...
    viewDetails(id) {
      ...
      //  .get(`http://10.100.164.35:8080/products/${id}`)
        ...
        .then((response) => {
          ...
          this.sendUserBehavior("click", this.currentProduct.name);
        })
        .catch((error) => {
          console.error("获取商品详情失败:", error);
        });
    },
    sendUserBehavior(action, name) {
...
//const tagsArray = name.split(',');
      ...
      axios
        .post("http://101.43.72.79:8081/api/behavior/upload", userBehavior)
        .then((response) => {
